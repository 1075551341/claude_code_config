...
> **设计源**：27 个 GitHub 仓库全量重新调研 | v6.0 全量整合
> **版本**：3.0 | **关联**：[design-v6.md](./design-v6.md) | [tasks-v6.md](./tasks-v6.md)
...
| FR-02.9.2 | ceo/designer/qa/security 条件触发           | MANIFEST gstack\_\* |
...
**来源**：用户要求第 9 点 + P0 发现#4（stop-quality-gate.py 裸 except 修复）

| ID      | 要求                                                               | 验收（EARS）                                                                                        |
| ------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| FR-21.1 | Hook 中禁止裸 `except:pass`，异常必须向上传播或显式处理并报告      | When Hook 捕获异常, the Hook shall 向上传播异常或显式报告错误详情给用户                             |
| FR-21.2 | Agent 执行失败必须明确报告错误详情，不静默吞掉                     | If Agent 执行失败, the 系统 shall 报告错误详情并询问用户解决方式                                    |
| FR-21.3 | 配置验证失败必须阻止操作并提示修复方案                             | If 配置验证失败, the 系统 shall 阻止当前操作并输出可操作的修复建议                                  |
| FR-21.4 | 铁律 R16 写入 `rules/CORE.md` 和 `CLAUDE.md`：禁止裸 `except:pass` | The 铁律 R16 shall 在 CORE.md 和 CLAUDE.md 中声明"禁止裸 except:pass，异常必须传播或显式处理并报告" |
| FR-21.5 | 全量扫描 hooks/ 确认无裸 `except:pass` 残留                        | While 执行 validate_config, the validate_config shall 扫描 hooks/目录并报告裸 except:pass 数量为零  |
| FR-21.6 | 子 Agent 异常不吞掉，主 Agent 必须收到子 Agent 错误并决定处理      | If 子 Agent 返回异常, the 主 Agent shall 接收异常详情并决定重试/报告/中止                           |
...
| NFR-11 | 可观测性 | 异常零静默吞掉；所有错误可追踪、可报告 | grep 裸 except=0 + Hook 异常传播率 100% |
...
- [ ] hooks/ 全量扫描裸 `except:pass` = 0
- [ ] R16 铁律写入 rules/CORE.md
- [ ] R16 铁律写入 CLAUDE.md 铁律表
- [ ] validate_config 新增裸 except 检查项
- [ ] Agent 执行失败报告机制验证
- [ ] 配置验证失败阻止操作并提示修复方案验证
...
- R16 铁律：禁止裸 `except:pass`，异常必须传播或显式处理并报告
...
| 错误静默掩盖         | 高   | R16 铁律 + validate_config 裸 except 扫描 + Hook 异常传播 |

---

_版本：2.5 | 日期：2026-06-04 | Phase 8 完成 + FR-21 错误暴漏 + R16 铁律_
