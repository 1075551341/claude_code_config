...
| 规则总纲 | sync → `~/.cursor/rules/*.mdc` | + `CURSOR-EDITOR.mdc`（Guard 部署） |
...
| codegraph 优先 | 自动提示 | `explore_router` + `CURSOR-EDITOR.mdc` |
...
|------|---------------|-------------|----------|
| 铁律 R17 / CORE | `rules/CORE.md` | `~/.cursor/rules/CORE.mdc` | `sync.ps1` |
| 路由 CLAUDE | `CLAUDE.md` | `~/.cursor/rules/CLAUDE.mdc` | `sync.ps1` |
| MCP 文档 | `rules/MCP.md` | `~/.cursor/rules/MCP.mdc` | `sync.ps1` |
...
| 编辑器专有规则 | Guard 模板 | `~/.cursor/rules/CURSOR-EDITOR.mdc` | `deploy-cursor-guard.ps1`（sync 已白名单保护） |
...
2. Hook：`explore_router`（Grep/Read/Glob 前提示，不阻断）
3. MCP：`codegraph` 必须在 Cursor Settings 启用且项目已 `codegraph init -i`
