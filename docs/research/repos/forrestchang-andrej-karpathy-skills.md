# forrestchang/andrej-karpathy-skills

> 层: 技能/实践 | 置信度: 高 | 刷新: 2026-06-24 | 上游: multica-ai/andrej-karpathy-skills | 来源: GitHub + juejin + mer.vin 三源交叉


## v10.5.1 delta (2026-07-17)
- **最新元数据**：193,293★；`pushed_at` 2026-04-20。
- **本地映射**：karpathy-guidelines skill integrated。
- **来源**：GitHub API（Tier-2）。
## 核心价值

- 176K+ Stars（2026-06；4 月 92K → 6 月 176K，3 个月翻倍）
- Karpathy 四原则：Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution
- 仅 1 个 CLAUDE.md 文件（~66 行），零可执行代码
- 最小 diff；拒绝过度抽象；每行修改可追溯到用户请求
- 目标驱动：imperative → verifiable goals（测试先行 + 循环验证）
- 与 CORE 编码规范互补；跨编辑器通用（Claude Code/Cursor/Codex）

## 证据

- [GitHub forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills)
- 176K+ Stars / 18K+ Forks；MIT License

## 本地映射

| MANIFEST concern | 路径 |
|------------------|------|
| coding_philosophy | `skills/karpathy-guidelines/SKILL.md` |
| rules_ref | `rules/CORE.md` Karpathy 段 |

## 吸收决策

**采纳** — L3 按需 Read；CORE 骨架引用。

## 互博检查

- vs CORE 铁律：互补，不重复全文

## v10.1 增量

- 无 upstream 变更；维持 L3 按需

## v10.3 增量

- Delta 刷新：Stars 92K → 176K（3 个月翻倍）
- 四原则已深度吸收到 CORE R1-R4 铁律
- 决策不变：L3 按需 Read；CORE 骨架引用

## v10.5 delta (2026-07-17)

- Stars：193,283；最新 Release：无正式 Release（`gh api`）。
- 保持现有采纳范围，无强制扩展。
