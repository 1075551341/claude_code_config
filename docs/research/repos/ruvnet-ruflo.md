# ruvnet/ruflo

> 层: L1 参考 | 置信度: 中 | 刷新: 2026-07-31 | 来源: GitHub + WebSearch 双源

## v10.5 delta (2026-07-17)

- **最新元数据**：64,700 stars；GitHub Release **v3.32.1**；`pushed_at` 2026-07-16T23:53:55Z。
- **自 2026-06-29 的变化**：从 v3.6.30 演进至 v3.32.1，蜂群编排仍在快速迭代；本轮未发现可安全吸收且不与既有编排层重叠的新能力。
- **本地吸收**：不变——**reference_only**；仅保留概念研究，不集成运行时，也不与 deer-flow / Superpowers 子代理并行部署。
- **双源**：GitHub API（stars/release/push）+ 仓库 README/既有 WebSearch 研究记录。


## v10.5.1 delta (2026-07-17)
- **最新元数据**：64,703★；Release **v3.32.1**（Codex integration）；`pushed_at` 2026-07-16T23:53:55Z。
- **漂移要点**：蜂群/多玩家编排持续迭代。
- **本地吸收 / 缺口**：**reference_only** 不变。
- **不吸收**：运行时集成。
- **双源**：GitHub API + 既有决策交叉。
## 核心价值

- Swarm 蜂群拓扑 + HNSW 向量
- SONA 自学习；100+ agents 概念
- 分布式编排参考架构
- 文档级概念吸收，不运行时集成

## 证据

- [GitHub ruvnet/ruflo](https://github.com/ruvnet/ruflo)

## 本地映射

| MANIFEST concern | 路径 |
|------------------|------|
| ruflo | `MANIFEST.yaml` → `status: reference_only` |

## 吸收决策

**reference_only** — 文档记录蜂群概念；不集成、不与 deer-flow 并行。

## 互博检查

- vs deer-flow / subagent：仅文档参考
- vs gstack agents：不合并运行时

## v10.1 增量

- 访谈二次确认 reference_only；无集成计划

## v10.3 增量

- Delta 刷新：v3.6.30（2026-06）；蜂群拓扑概念持续演进
- 决策不变：reference_only；不集成、不与 deer-flow 并行
