...
- **最新元数据**：141,985★；`pushed_at` 2026-07-12。
- **本地映射**：reference_only；不直接注入运行时。
- **来源**：GitHub API（Tier-2）。
## 核心价值

- 工具描述宜简；护栏放系统提示
- 各 AI 工具 system prompt 参考
- 避免 tool description 膨胀浪费 token
...
| MANIFEST concern | 路径 |
|------------------|------|
| 简洁原则 | `rules/CORE.md` 输出格式 |
| MCP 描述 | `.mcp.json` 精简描述 |

## 吸收决策

**采纳概念** — CORE 简洁优先；不复制 prompt 全文。

## 互博检查

- vs 冗长 rules：R10 简洁优先
...
- Stars：141,984；最新 Release：无正式 Release（`gh api`）。
- 保持现有概念采纳范围，无强制扩展。
