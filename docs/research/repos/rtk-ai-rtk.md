...
- **本地升级**：v0.40.0 → **v0.44.1**（用户显式要求重装；SHA256 校验通过，`rtk --version`/`gain`/`git status` 压缩实测通过）
- v0.44+ 新增 `[rtk] No hook installed` 提示：本地走 `pre-rtk-rewrite.py`，**不运行** `rtk init -g`（防双重重写），已在 `RTK.md` 注明
- MANIFEST `L2_optimization` 版本对齐 0.44.1
