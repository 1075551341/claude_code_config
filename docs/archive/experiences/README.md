# Experiences 经验库（已归档 · 废止）

> ⛔ **v11.0.0 废止归档**（2026-08-12）：本目录自 `~/.claude/experiences/` 整体迁入 `docs/archive/`，不再参与任何路由/加载。
> 学习产物统一由 claude-mem observation 承接（R18 记忆柱）。
> 注意：`patterns/2026-06-29-dual-engine-code-exploration.md` 等 cbm 双引擎相关内容与现行策略**冲突**（codebase-memory 已于 v10.10 永久禁用），仅作历史参考，勿照做。
> 配套脚本 `collect-experience.ps1` 一并归档于此。
