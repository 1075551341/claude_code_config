...
L3 洞察 — codegraph(R17 常驻) + codebase-memory(L4 场景强制·Claude 按需) + Firecrawl/Exa(外部搜索)  # UA removed v10.5
...
**claude-mem 三层**：search索引 → 识别关键IDs → fetch详情。**cbm 场景强制**：架构/ADR/变更/跨服务必须调用，未调用标 `DONE_WITH_CONCERNS`。
