...
description: 负责后端开发任务。当需要实现后端功能、开发REST API接口、创建数据库模型、编写服务端逻辑、设计数据库表结构、实现认证授权、处理文件上传、开发WebSocket服务、编写中间件、实现消息队列、集成第三方服务API、创建定时任务时调用此Agent。触发词：后端、API、接口、路由、数据库、服务端、Express、Koa、Fastify、FastAPI、Flask、Django、Prisma、TypeORM、Redis、JWT、中间件、Controller、Service、Repository。
model: inherit
color: green
tools:
  - Read
