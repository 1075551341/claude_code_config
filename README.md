...
├── README.md           # 本文件（总览）
│
├── agents/             # 91 个专业 AI 代理角色（见 agents/README.md）
├── skills/             # 165 个可复用技能模块（各技能目录下 SKILL.md）
├── rules/              # 17 个代码与行为规范
├── hooks/              # 47 个 .py（45 个生命周期钩子 + `_editor_hook_launcher.py` + `_editor_safe_guard.py`；CLI；编辑器内由 editor-guard 快速跳过）
...
91 个专业领域 AI 代理，覆盖软件开发全流程。

### 按领域分类

| 领域            | 智能体                                                                            |
| --------------- | --------------------------------------------------------------------------------- |
| **前端开发**    | frontend-developer, mobile-developer, react-reviewer, ux-design-expert, typescript-pro   |
| **后端开发**    | backend-developer, nodejs-reviewer, python-pro, python-reviewer                   |
| **数据库**      | database-expert, data-engineer                                        |
| **测试质量**    | qa-engineer, code-review-workflow                      |
| **安全性能**    | security-reviewer, performance-analyzer                                            |
| **架构设计**    | architect                                           |
| **运维 DevOps** | devops-engineer, observability-engineer, incident-responder |
| **工程效能**    | git-expert, refactoring-expert, build-error-resolver                                 |
| **AI/数据**     | ai-engineer, agentic-orchestrator, ml-engineer                                    |
| **文档沟通**    | docs-expert, ppt-creator, business-writing            |
| **专项领域**    | payment-integration, planning-expert, mermaid-expert, systematic-debugging                    |
...
165 个独立、可复用的技能模块，AI 可直接调用处理特定任务。
...
| **skill-learning-system** | 技能学习   | 模式提取、技能创建、持续学习   |
...
16 个核心规则文件，AI 编码时自动生效：

| 规则文件              | 作用范围 | 主要内容                           |
| --------------------- | -------- | ---------------------------------- |
| **RULES_CORE.md**     | 全局通用 | 最佳实践、Clean Code、注释规范、错误处理、测试 |
...
| **RULES_CSHARP.md**   | C#       | C# / .NET 开发规范                 |
| **RULES_DART.md**     | Dart     | Dart / Flutter 开发规范            |
| **RULES_GO.md**       | Go       | Go 语言开发规范                    |
| **RULES_RUST.md**     | Rust     | Rust 语言开发规范                  |
