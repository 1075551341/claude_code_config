---
description: Claude 全局配置 — 五柱×五阶段×三横切
trigger: always_on
---
# Claude 全局配置

> 五柱×五阶段×三横切 | 路由→`SPEC.md` | 归属→`MANIFEST.yaml` | **v10.1**

**五柱**：Superpowers(方法论) | GSD(上下文) | OpenSpec(规格) | gstack(审查) | claude-mem(记忆)
**三横切**：L1 ECC模式(cherry-pick)+deer-flow | L2 RTK+caveman+阈值 | L3 codegraph+Firecrawl/Exa（UA disabled）
...
代码探索：**codegraph → Grep → Read**（须 `codegraph init`；UA disabled，见 ADR-2026-06-16）
...
**阈值**：<70%正常 | 70%压缩 | 90%强制 | GSD **70%逻辑断点**（任务边界） | ⛔100%
...
**OpenSpec OPSX**：`/opsx:propose` → `continue|ff` → `apply` → `verify` → `sync` → `archive` | CLI: `openspec init --tools cursor`（profile: core，含 sync）
...
| 内容 | 位置 |
|------|------|
| 法典/插件/加载 | SPEC.md (v10.1) |
| 调研 SSOT | docs/research/30-repo-deep-research-v10.md + repos/ |
| v10.1 设计/规格 | spec/claude-config-integration/design-v10.1.md |
