...
- **运行时**：`which_pwsh()` 只认 `pwsh`；live `scripts/*.ps1` 无 `#Requires -Version 5.1`。Cursor Guard **1.2.16**（`sync_runner` 只 spawn pwsh；缺则 BLOCKED）。
- **validate_config V20**：钉 `powershell_min: "7.5"`、`package_manager: pnpm`、`pnpm_major: "11"`、`missing_tool: BLOCKED`、`python.installer: uv`、脚本 Requires、`which_pwsh` 不回退、mcp.json/Cursor 片段不得 `command: powershell`、Guard `sync_runner` 须解析 pwsh、CLAUDE.md R15 不含「兜底」。
- **审查修正**：活 MCP spawn 从 `powershell` 改为 `pwsh`（包装脚本 `#Requires 7.5` 否则会拒启）；V14 版本史取现行 Guard 戳（不再钉死 v20.19）；bootstrap 对 pyproject/requirements 回报 `uv`。
