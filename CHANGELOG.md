...
- **测试**：`hooks/tests/test_git_r19.py`（复合命令 / `bash -c`/`eval`/`timeout`/`nice` 包装 / `checkout -` / `.` / `HEAD file` / poetry.lock / `python -m pip`）+ bash fixture；`test-cursor-guard-hooks.py` 分支 ask（含复合与包装命令）与路径还原。
