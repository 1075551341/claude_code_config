...
| plan-execution | 计划执行、实施步骤、执行跟踪 |
| writing-plans | 计划编写、实施计划、开发计划 |
| subagent-driven-development | 子代理驱动、Agent协作、并行开发 |
| troubleshooting-guide | 故障排除、问题诊断、排查指南 |
| using-skills | 技能使用、技能指南、技能说明 |
| writing-skills | 技能编写、技能开发、SKILL.md |
...
| web-testing | Playwright、Web测试、自动化测试 |
...
### Windsurf
将 `SKILL.md` 内容添加至 `.windsurfrules` 文件。
...
_最后更新：2026-04-08（同步优化技能库，147个技能全部标准化，两个目录完全同步）_
## 新增技能
...
### 浏览器自动化
- **首选**: Playwright MCP (mcp6_*) - E2E测试、表单填写
- **备选**: Puppeteer MCP (mcp8_*) - PDF生成

### 搜索与信息
- **语义搜索**: Exa MCP (mcp1_*)
- **直接获取**: Fetch MCP (mcp2_*)
- **GitHub文档**: DeepWiki MCP (mcp0_*)

### 文件与Git
- **文件操作**: Filesystem MCP (mcp4_*)
- **Git操作**: Git MCP (mcp5_*)
- **设计稿**: Figma MCP (mcp3_*)
