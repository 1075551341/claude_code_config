...
└─ 会话终验 R20（任何有过编辑的会话必须：对照原始请求 满足/遗漏/错改）
```
...
对照用户**原始请求**输出清单，不是把任务重做一遍；测试/lint 证据不能代替本项。
...
未输出不得声称完成。Stop 硬门检测标记：`会话终验` 或 `R20`，且同时含 `遗漏` 与 `错改`。
...
**场景D：非功能变更回归保持（v10.15 新增，任何非功能变更必须）**

```
□ 变更性质判定：本变更是否为重构/格式/配置/重命名等非功能变更
□ 是 → 必须核验原功能行为不变：运行既有测试或冒烟核验核心路径
□ 禁止"应该没影响"式断言；证据不足 → verify_tier 升全量
