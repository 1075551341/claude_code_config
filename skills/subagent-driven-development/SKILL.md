...
description: 用于子代理驱动开发模式，每个任务由新子代理执行并经过两阶段审查，含并行调度能力。当有实施计划需要执行、需要将任务分发给子代理、复杂功能需要分步实现、并行调度多个子Agent时使用。触发词：子代理开发、subagent、subagent-driven、任务分发、并行开发、代理协调、执行计划、任务拆解、子代理审查、多代理协作、并行Agent、批量处理、并行处理、同时执行、并发Agent、Agent调度、多任务并行。
---
