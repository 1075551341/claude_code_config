...
> 上游 v6.3.0「Ceremony scales to the task」：仪式随任务规模伸缩，但 **HARD-GATE 一律不减**。

| 级别 | 判定 | 仪式 |
|------|------|------|
| spike | 探索性/可丢弃验证（试 API、跑通 demo） | 免访谈+免设计文档；探索结论直接回贴对话，仍须用户点头后才写正式代码 |
| bounded | 边界清晰的单点改动（≤2 文件、方案唯一） | 可合并访谈与设计为单段确认；免独立设计文档，直接进 writing-plans 轻量计划 |
| architectural | 跨模块/新架构/多方案取舍 | 完整流程：逐问澄清 → 方案对比 → 设计文档 → 自审 → 用户批准 |

判定存疑时升一级处理；spike 产出若要转正为正式功能，必须补走对应级别仪式。
