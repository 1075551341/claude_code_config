...
triggers: [重构, 优化函数, 清理代码, 拆分过长函数, 消除代码异味]
metadata:
  version: 1.0.0
loading_tier: L3
...
- **执行重构**（默认）：按本文件主流程（基线测试→风险评估→分步重构→收益报告）。
- **前端提案模式**：用户只要方案不要改码、且为前端单文件/微模块（.vue/.tsx/.js 圈复杂度高、难测试、逻辑耦合）→ Read [frontend-refactor-proposal.md](frontend-refactor-proposal.md)（原独立技能 frontend-refactor-proposer，输出保守/均衡/激进三套策略决策报告，严禁直接改码）。
