...
> **L3**：记忆查询或维护任务时 Read。⑤学习默认 claude-mem pattern 提取；`skills/instinct-learning` 仅显式「提取模式」信号时按需启用。
...
R18 的另一半：学习产物**写入** observation 规范由 [`instinct-learning`](../instinct-learning/SKILL.md) §⑤ 持久化定义；日常写入直接按下述规范执行。
