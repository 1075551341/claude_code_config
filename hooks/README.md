...
`_editor_hook_launcher.py` **v2.0**：settings.json 中 Hook 经 launcher 调用；**GetConsoleWindow()==0**（无控制台）时在编辑器内快速跳过。

```python
# 主判定（Windows）
if ctypes.windll.kernel32.GetConsoleWindow() == 0:
    print(json.dumps({"continue": True, "skipped": True}))
    sys.exit(0)
# 否则 subprocess 转调真实 hook 脚本
```

部署/修复：`powershell -ExecutionPolicy Bypass -File ~/.claude/scripts/fix.ps1 -Fix`
...
## 统计（`hooks/` 根目录 `.py`，2026-05-23）

| 类型 | 数量 | 说明 |
| ---- | ---- | ---- |
| PreToolUse | 8 | settings.json 已注册 |
| PostToolUse | 6 | settings.json 已注册 |
| Stop | 7 | settings.json 已注册 |
| SessionStart | 1 | session-start-bootstrap |
| PreCompact | 1 | pre-compact-state |
| Launcher | 1 | `_editor_hook_launcher.py`（非业务 hook） |
| **总计** | **24** | 经 launcher 路由 23 个业务 hook |
