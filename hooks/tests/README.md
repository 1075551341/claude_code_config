...
| bash_git_compound_checkout_b_blocked.json | pre-bash-guard R19 | exit 2（`cd … && git checkout -b` 复合命令） | v11.4.13 |
| bash_git_checkout_dot_allowed.json | pre-bash-guard R19 | exit 0（`git checkout .` 路径还原） | v11.4.13 |
